...
|---|---|---|---|
| ACC-BLD-001–007 | PASS | doctor, typecheck, 72 tests, package, extension builds | |
...
| ACC-EMB-* | PASS local | Vendored Granite ONNX + hash fallback + ExactScan | |
...
- HashEmbeddingService fallback when ONNX unavailable
- Mock provider via `OMAKASE_MOCK_PROVIDER=1`
